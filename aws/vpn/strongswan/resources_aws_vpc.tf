resource "aws_security_group" "main" {
  ...
  name_prefix            = "${local.name_prefix}-sg-vpn-"
...
resource "aws_vpc_security_group_ingress_rule" "ikev2_ipv6" {
  ...
  to_port           = 500
  ip_protocol       = "udp"
  cidr_ipv6         = local.wildcard_ipv6

  tags = {
    Name = "${local.name_prefix}-sg-ingress-ipv4_udp_500-internet"
  }
}
...
resource "aws_vpc_security_group_ingress_rule" "ikev2_nat_t_ipv6" {
  ...
  tags = {
    Name = "${local.name_prefix}-sg-ingress-ipv4_udp_4500-internet"
  }
}
...
resource "aws_vpc_security_group_egress_rule" "all_ipv4" {
  ...
  tags = {
    Name = "${local.name_prefix}-sg-egress-ipv4-internet"
  }
}
...
resource "aws_vpc_security_group_egress_rule" "all_ipv6" {
  ...
  tags = {
    Name = "${local.name_prefix}-sg-egress-ipv6-internet"
  }
}
...
resource "aws_route" "vpn_clients" {
  ...
  destination_cidr_block = local.client_pool
